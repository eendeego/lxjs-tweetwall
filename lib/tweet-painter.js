...
(function (exports) {
  ...
  function browserImageFetcher(url) {
    var img = new Image();
    img.src = url;
    return img;
  }
  ...
  var Painter = exports.Painter = function (canvas, options) {
    ...
    self.imageFetcher = options.imageFetcher || browserImageFetcher;
    ...
    function initContext(ctx) {
      ...
      self.lineHeight = self.textHeight * 1.4;
  ...
  Painter.prototype.getImage = function (url) {
    ...
    var img = self.imageCache[url] = self.imageFetcher(url);
    return img;
  };
  ...
  Painter.prototype.paint = function (time) {
    ...
    while (y < self.height && idx >= 0) {
      ...
      var metrics = self.queue[idx].metrics;
      var tweetHeight;
      if (!metrics) {
        tweetHeight = USER_MARGIN.top + self.userHeight + Math.max(USER_MARGIN.bottom, TEXT_MARGIN.top) + TEXT_MARGIN.bottom;
        ...
        self.queue[idx].metrics = metrics;
      } else {
        tweetHeight = metrics.tweetHeight;
      }
      ...
      ctx.strokeStyle = '#000';
      ctx.beginPath();
      ctx.moveTo(0, tweetHeight - 0.5);
      ctx.lineTo(self.width, tweetHeight - 0.5);
      ctx.stroke();
      ...
      ctx.font = 'normal ' + self.userHeight +  'px "Lucida Grande", sans';
      ...
      for (i = 0; i < metrics.lines.length; i++) {
        ctx.fillText(metrics.lines[i],
